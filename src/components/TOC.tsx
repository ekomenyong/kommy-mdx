...
const useIntersectionObserver = (setActiveId: Dispatch<SetStateAction<string | undefined>>) => {
...
const TableOfContents: FC<TOCProps> = ({ source }) => {
	...
	return (
		<div className="mt-4 mb-0 cursor-pointer xl:border-b xl:border-gray-200 xl:dark:border-gray-700">
			<Disclosure as="div" className="flex flex-col items-start justify-center" defaultOpen={true}>
				{({ open }) => (
					<>
						<dt>
							<Disclosure.Button
							...
							>
								<span className="uppercase animated-underline">Table of Contents</span>
						...
						<Disclosure.Panel as="dd" className="flex flex-col items-start justify-start">
							{headings.map((heading, index) => {
								return (
									<button
										key={index}
										type="button"
										className={`${heading.id === activeId ? 'font-medium' : 'font-normal'} ${
